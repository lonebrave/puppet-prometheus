...
define prometheus::daemon (
  ...
  Stdlib::Absolutepath $archive_bin_path  = "/opt/${name}-${version}.${os_type}-${arch}/${name}",
  ...
  case $install_method {
    'url': {
      if $download_extension == '' {
        file { "/opt/${name}-${version}.${os_type}-${arch}":
          ensure => directory,
          owner  => 'root',
          group  => 0, # 0 instead of root because OS X uses "wheel".
          mode   => '0755',
        }
        -> archive { "/opt/${name}-${version}.${os_type}-${arch}/${name}":
          ensure          => present,
          source          => $real_download_url,
          checksum_verify => false,
          before          => File["/opt/${name}-${version}.${os_type}-${arch}/${name}"],
        }
      } else {
